...
// Initialize parameters for teei.
...
{
	unsigned int val = gicd_read_itargetsr(
				get_plat_config()->gicd_base, id);

	gicd_v3_set_irouter(get_plat_config()->gicd_base, id,
				gic_mpidr_to_affinity(read_mpidr()));
...
void teei_gic_setup(void)
{
	...
	for (unsigned int index = 0;  index < SEC_EXC_CNT; index++) {
		...
		gicd_set_ipriorityr(gicd_base, sec_exc[index],
...
void teei_triggerSgiDump(void)
{
	...
	gicd_set_ipriorityr(rdist_sgi_base, FIQ_SMP_CALL_SGI,
				GIC_HIGHEST_SEC_PRIORITY);

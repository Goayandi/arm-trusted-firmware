...
void atf_sched_clock_init(unsigned long long normal_base,
...
/*
 * Return: 0 - Trying to disable the CPUXGPT control bit, and not allowed
 * to disable it.
						...
						unsigned int reg_value)
{
	...
	if (reg_addr == CTL_BASE) {
		...
		if (idx == 0) {
			...
			if (ctl_val & 1) {
				/* if enable bit already set, then
				 * bit 0 is not allow to set as 0
				 */
				if (!(reg_value & 1))
					return 0;
			}

...
static const struct OCP_setting BigOCP_setting_table[] = {
...
static const struct OCP_setting Cluster0_OCP_setting_table[] = {
...
static const struct OCP_setting Cluster1_OCP_setting_table[] = {
...
static const unsigned short DREQ_bit_map[] = {
	600, 861,  874,  887,  901,  914,  927,  940,  953,  967,  980,  993,
	1006, 1020, 1033, 1046, 1059, 1073, 1086, 1099, 1112, 1126, 1139, 1152,
	1165, 1178, 1192, 1205, 1218, 1231, 1245, 1258, 1271, 1284, 1298, 1311,
	1324, 1337, 1351, 1364, 1377, 1390, 1421, 1417, 1430, 1443, 1456, 1470,
	1483, 1496, 1509, 1523, 1536, 1549, 1562, 1576, 1589, 1602, 1615, 1628,
	1642, 1655, 1668, 1681
};
...
int BigOCPConfig(int VOffInmV, int VStepInuV)
{
	...
	for (i = 0;
		i < sizeof(BigOCP_setting_table) / sizeof(struct OCP_setting);
		i++)
		ocp_write(BigOCP_setting_table[i].addr,
			  BigOCP_setting_table[i].value);
	...
	if (PowerCal != 0x0)
		for (i = 14; i < 20; i++)
			Write_Big_RCoef(
				BigOCP_setting_table[i].addr,
				(BigOCP_setting_table[i].value >> 16),
				(BigOCP_setting_table[i].value & 0x0000FFFF),
				PowerCal);
 ...
 * 10. if Sleect=OCPI, write OCPIEn =1, if Select=FPI write FPIEn = 1,
 *     if Select=ALL write both = 1
 */
int BigOCPEnable(int OCPMode,  int Units, int ClkPctMin, int FreqPctMin)
{
	...
	if (OCPMode == OCP_ALL) {
		/*
		 * Read OCPIEn & FPIEn. If selected ones already enabled,
		 * exit/error -2
		 */
 ...
 * 3. Write ALL LkgMonInit = 0 (IRQEn = 0)
 * 4. Re-configure OCPAPBCFG[16] to previous setting
 *
...
void BigOCPDisable(void)
{
	...
	for (Temp = 0;
		Temp < sizeof(BigOCP_setting_table) /
			sizeof(struct OCP_setting);
		Temp++)
		ocp_write(BigOCP_setting_table[Temp].addr,
				BigOCP_setting_table[Temp].value);
...
int BigOCPIntLimit(int Select, int Limit)
{
	...
	if (Select == TotLkgMax) {
		...
		printf("ocp: X=%d LM=%x CFG12:13=%x:%x\n", Limit, LkgMax,
			ocp_read(OCPAPBCFG12), ocp_read(OCPAPBCFG13));
#endif
		...
		printf("ocp: X=%d WNA=%x CFG10:11=%x:%x\n", Limit, WAMinAct,
			ocp_read(OCPAPBCFG10), ocp_read(OCPAPBCFG11));
#endif
		...
		printf("ocp: X=%d WNP=%x CFG11:12=%x:%x\n", Limit, WAMinPred,
			ocp_read(OCPAPBCFG11), ocp_read(OCPAPBCFG12));
#endif
...
//3. Convert Edge to CaptureEdge, Count to CaptureHoldOff,
//   Trig to CaptureTrigSel
...
int BigOCPCapture(int EnDis, int Edge, int Count, int Trig)
{
	...
	if (EnDis == OCP_DISABLE)
		...
		ocp_write(OCPAPBCFG03, 0x0);
// 3. set Trig = 0
		ocp_write(OCPAPBCFG03, ((Edge << 24) | Count));
// 4. Write CaptureEdge, CaptureHoldOff
		ocp_write(OCPAPBCFG03, ((Edge << 24) | (Trig << 20) | Count));
// 5. Write CaptureTrigSel
	}
...
{
	unsigned int i, Valid = 0, Temp = 0, Avg = 0;
	...
	for (i = 0; i < Count; i++) {
		...
		if (ocp_read_field(OCPAPBSTATUS01, 0:0) == 1) {
			Temp = (ocp_read_field(OCPAPBSTATUS03, 18:0) * 1000) >>
				12;
...
int LittleOCPConfig(int Cluster, int VOffInmV, int VStepInuV)
{
	...
	int Cpu2LkgTrim, Cpu3LkgTrim, PowerCal;
	int i;
...
//Calculate & write VOff & VStep
	...
	if (Cluster == OCP_LL) {
		for (i = 0;
			i < (sizeof(Cluster0_OCP_setting_table) /
				sizeof(struct OCP_setting));
			i++)
			ocp_write(Cluster0_OCP_setting_table[i].addr,
					Cluster0_OCP_setting_table[i].value);
...
// calculate corrected RCoef’s
		...
		if (PowerCal != 0x0) {
			for (i = 1; i < 27; i++)
				Write_Little_RCoef(
					Cluster0_OCP_setting_table[i].addr,
					Cluster0_OCP_setting_table[i].value,
					PowerCal);
		}
		...
		ocp_write((MP0_OCP_LKG_CTRL),
				((Cpu3LkgTrim << 16) | (Cpu2LkgTrim << 12) |
					(Cpu1LkgTrim << 8) |
					(Cpu0LkgTrim << 4) |
					TopLkgTrim));
		...
		for (i = 0;
			i < (sizeof(Cluster1_OCP_setting_table) /
				sizeof(struct OCP_setting));
			i++)
			ocp_write(Cluster1_OCP_setting_table[i].addr,
					Cluster1_OCP_setting_table[i].value);
		...
		if (PowerCal != 0x0) {
			for (i = 1; i < 27; i++)
				Write_Little_RCoef(
					Cluster1_OCP_setting_table[i].addr,
					Cluster1_OCP_setting_table[i].value,
					PowerCal);
		}
...
//  RCoeff = mp0_ocpcpuroff[31:0], mp0_ocpcpur0[15:0] ~ mp0_ocpncpu8[15:0],
//  mp0_ocpncpulkg, mp0_ocp_cpurlkg
...
//  Return to previous settings.
	...
	if (Cluster == OCP_LL) {
		...
		for (i = 0; i <
			(sizeof(Cluster0_OCP_setting_table) /
			sizeof(struct OCP_setting));
			i++)
			ocp_write(Cluster0_OCP_setting_table[i].addr,
					Cluster0_OCP_setting_table[i].value);
...
//2. Write FAct & VAct"
	...
	if (Cluster == OCP_LL) {
		...
		// Write FAct
		Temp = FreqMHz << 12;
		// percentage of MAX freqency
		ocp_write(MP0_OCP_IDVFS_FACT,
				(Temp/MP0_MAX_FREQ_MHZ) * 100);
		...
		Temp = FreqMHz << 12;
		// percentage of MAX freqency
		ocp_write(MP1_OCP_IDVFS_FACT,
			(Temp/MP1_MAX_FREQ_MHZ) * 100);
...
// For Cluster:
// 1. Convert Limit to WAMaxAct, WAMinAct, LkgMax, OCCGMin
//    (exit/return error -1 if out of range)
...
//7. Write CaptureTrigSel
	...
	if (Cluster == OCP_LL) {
		...
		if (EnDis == OCP_DISABLE)
			...
			LittleOCPCapture_Trig(MP0_OCP_CAP_CTRL,
				MP0_OCP_CAP_HOLDOFF, Count, Edge, Trig);
		...
		if (EnDis == OCP_DISABLE)
			...
			LittleOCPCapture_Trig(MP1_OCP_CAP_CTRL,
				MP1_OCP_CAP_HOLDOFF, Count, Edge, Trig);
	}
...
int LittleOCPAvgPwr(int Cluster, int EnDis, int Count)
{
	...
	if (Cluster == OCP_LL) {
		...
		ocp_write_field(MP0_OCP_DBG_STAT,
				0:0, ~ocp_read_field(MP0_OCP_DBG_STAT, 0:0));
	} else if (Cluster == OCP_L) {
		...
		ocp_write_field(MP1_OCP_DBG_STAT,
				0:0, ~ocp_read_field(MP1_OCP_DBG_STAT, 0:0));
	}
...
unsigned int LittleOCPAvgPwrGet(int Cluster)
{
	...
	if (Cluster == OCP_LL) {
		if (ocp_read_field(MP0_OCP_DBG_STAT, 31:31) == 1) {
			AvgAct = ((((((unsigned long long)
					ocp_read(MP0_OCP_DBG_ACT_H) << 32) +
					(unsigned long long)
					ocp_read(MP0_OCP_DBG_ACT_L)) * 32) /
					(unsigned long long)
					ocp_read_field(MP0_OCP_DBG_IFCTRL1,
					21:0)) * 1000) >> 12;
		...
		if (ocp_read_field(MP1_OCP_DBG_STAT, 31:31) == 1) {
			AvgAct = ((((((unsigned long long)
					ocp_read(MP1_OCP_DBG_ACT_H) << 32) +
					(unsigned long long)
					ocp_read(MP1_OCP_DBG_ACT_L)) * 32) /
					(unsigned long long)
					ocp_read_field(MP1_OCP_DBG_IFCTRL1,
					21:0)) * 1000) >> 12;
...
/*
 * Cluster 0,1 DREQ
 * 1. If enable, set 0x1000_1000 to 4'b1111 when CA7_DVFS & CA7_DVDDRAM both
 *    at highest voltage
 * 2. If disable, set 0x1000_1000 to 4'b0000 when one of them intend to leave
 *    highest voltage"
 */

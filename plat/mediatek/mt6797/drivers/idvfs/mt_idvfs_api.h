...
#else
#define ptp3_reg_read(addr)		(*(unsigned int *)(addr))
#define ptp3_reg_write(addr, val) \
		(*(unsigned int *)(addr) = (unsigned int)(val))
#endif

#define ptp3_shift(val, bits, shift)	((val & ((0x1U << bits) - 1)) << shift)
#define ptp3_reg_write_bits(addr, val, bits, shift) \
		ptp3_reg_write(addr, ((ptp3_reg_read(addr) & \
		~(ptp3_shift(0xffffffffU,  bits, shift))) | \
		ptp3_shift(val, bits, shift)))
#define ptp3_reg_read_bits(addr, bits, shift) \
	((ptp3_reg_read(addr) >> shift) & ((0x1U << bits) - 1))
 ...
 * 3:0	sw_tck	Software JTAG control for top-level GatewayTAP, 1, 2 ,4 ,8
 *	for sub_chains 0/1/2/3
 */
...
#define rg_idvfs_debug_mux_w(val) \
			ptp3_reg_write_bits(IDVFS_CTL0, val, 4, 12)
#define rg_idvfs_debug_mux_r()	ptp3_reg_read_bits(IDVFS_CTL0, 4, 12)
#define rg_armpll_posdiv_w(val) \
			ptp3_reg_write_bits(ARMPLL_CON0_PTP3, val, 3, 12)
#define rg_armpll_posdiv_r()	ptp3_reg_read_bits(ARMPLL_CON0_PTP3, 3, 12)
#define rg_armpll_sdm_fra_en_w(val) \
			ptp3_reg_write_bits(ARMPLL_CON0_PTP3, val, 8, 0)
#define rg_armpll_sdm_fra_en_r() \
			ptp3_reg_read_bits(ARMPLL_CON0_PTP3, 8, 0)
...
#define rg_armpll_sdm_pcw_w(val) \
			ptp3_reg_write_bits(ARMPLL_CON1_PTP3, val, 31, 0)
#define rg_armpll_sdm_pcw_r()	ptp3_reg_read_bits(ARMPLL_CON1_PTP3, 31, 0)
#define rg_armpll_sdm_pcw_chg_w(val) \
			ptp3_reg_write_bits(ARMPLL_CON1_PTP3, val, 1, 31)
...
#define rg_sw_vproc_wait_to_w(val) \
			ptp3_reg_write_bits(IDVFS_SWVOL, val, 16, 16)
#define rg_sw_vproc_wait_to_r()	ptp3_reg_read_bits(IDVFS_SWVOL, 16, 16)
#define rg_sw_vsram_w(val) \
			ptp3_reg_write_bits(IDVFS_SWVOL, val, 8, 8)
...
#define rg_sw_avgfreq_len_w(val) \
			ptp3_reg_write_bits(IDVFS_SWAVG, val, 3, 4)
...
/* read only  */
#define rg_sw_avgfreq_r()	ptp3_reg_read_bits(IDVFS_SWAVG, 15, 16)
...
/* Fmax = 500~3000, Vproc */
extern int API_BIGIDVFSENABLE(unsigned int Fmax,
			unsigned int vproc_mv_x100,
			unsigned int vsram_mv_x100);
extern int API_BIGIDVFSDISABLE(void);

/* 250~3000(MHz) */
extern int API_BIGPLLSETFREQ(int Freq);
/* add for protect pod div */
extern int API_BIGIDVFSSWREQ(unsigned int swreq_reg);
/* 60000 ~ 120000 */
extern int BigSRAMLDOSet(int mVolts_x100);

...
/* define setting */
struct IDVFS_setting {
	unsigned int addr;
	unsigned int value;
};

static const struct IDVFS_setting idvfs_setting_table[] = {
	/* (((0x01 << 16) * 2500) / 26) / 100); Fmax setting */
	{IDVFS_PLLINDEX,	0x0000f627},
	/* ((400L * 100 * (1 << 12)) / 2500)); */
	{IDVFS_PLLSTEP,		0x00010000},
	/* ((10L * 100 * (1 << 12)) / 2500)); */
	{IDVFS_PLLOVSHT,	0x00000666},
	/* PLL settle time */
	{IDVFS_PLLSETTL,	0x00000068},
	/* PLL period time for SSC */
	{IDVFS_PLLSSCCTRL,	0x02080018},
	/* PLL for SSC ppm */
	{IDVFS_PLLSSCDEL,	0x00000069},
	/* PLL for SSC ppm */
	{IDVFS_PLLSSCDEL1,	0x00000069},
	/* mt6313 = 0x000d000d, ds9214 = 0x0001001b */
	{IDVFS_VSETTLE,		0x0001001b},
	/* mt6313 = 0x50100030, da9214 = 0x460a001e */
	{IDVFS_VDELTA,		0x460a001e},
	/* vproc wait = 0xffff, vsarm fixed = 1180mv, vproc by input */
	{IDVFS_SWVOL,		0xffff6c46},
	/* default 750/2500 = 30% */
	{IDVFS_SWREQ,		0x0001e000},
	/* H2L THD PRCT = 40.799805% * 2500 = 1020MHz */
	{ARMPLL_CON6_PTP3,	0x00028ccc},
	/* L2H THD PRCT = 59.599854% * 2500 = 1490MHz */
	{ARMPLL_CON7_PTP3,	0x0003b999},
	/* L Clamp PRCT = 20.199951% * 2500 = 505MHz */
	{ARMPLL_CON8_PTP3,	0x00014333},
	/* H Clamp PRCT = 116% * 2500 = 2900MHz*/
	{ARMPLL_CON9_PTP3,	0x00074000},
	/* SW AVG enable, time = 1.260ms */
	{IDVFS_SWAVG,		0x00000021},
};
...
int UDIRead(unsigned int reg_value, unsigned int t_count)
{
	...
	printf("CH=%d, UDI=0x%x, TRST=%d, TMS=%d, TDI=%d, Count=%d, ",
		sub_chains, ptp3_reg_read(UDI_PTP3_UDI_REG),
		i_trst, i_tms, i_tdi, t_count);
#endif
 ...
 * Fmax = 500~3000, Vproc,
 */
int API_BIGIDVFSENABLE(unsigned int idvfs_ctrl,
			unsigned int vproc_mv_x100,
			unsigned int vsram_mv_x100)
{
	int i;
	...
	/*
	 * enable idvfs also enable SW channel, and Vproc timeout interrupt,
	 * debug mux = 7
	 */
	if (idvfs_ctrl == 2500)
	...
	for (i = 0;
		i < (sizeof(idvfs_setting_table) /
			sizeof(struct IDVFS_setting));
		i++)
		ptp3_reg_write(idvfs_setting_table[i].addr,
					idvfs_setting_table[i].value);

	/* Read Vproc from SOC level
	 *  how to convert format, It is the same format as IDVFS_SWVOL[7:0]
	 ...
	 * 16. Read sramldo_vosel:
	 * how to convert format, vosel to IDVFS_SWVOL[15:8] is in this table.
	 */

	printf("IDVFS_PLLINDEX = 0x%x, rg_armpll_sdm_pcw_r = 0x%x,",
		ptp3_reg_read(IDVFS_PLLINDEX), rg_armpll_sdm_pcw_r());
	printf("IDVFS_SWREQ = 0x%x\n", ptp3_reg_read(IDVFS_SWREQ));
...
int API_BIGIDVFSDISABLE(void)
{
	...
	do {
		...
		udelay(2);
	} while ((ptp3_reg_read(IDVFS_DEBUGOUT) & 0x02) ==  0);
	/* wait idvfs_debugout[1] == 1 */

	/* set ARMPLL freq */
	rg_idvfs_debug_mux_w(0x07);
	/* idvfs_debugout[30:0] */
	rg_armpll_sdm_pcw_w((ptp3_reg_read(IDVFS_DEBUGOUT) & 0x7fffffff));
	...
	/* idvfs_debugout[2:0] */
	rg_armpll_posdiv_w((ptp3_reg_read(IDVFS_DEBUGOUT) & 0x7));
	/* rg_armpll_posdiv_w(1); */ /* div 2 */
	printf("iDVFS disable POSDIV = %x.\n",
		((ptp3_reg_read(IDVFS_DEBUGOUT) & 0x7)));

	/*
	 * due to DA9214 10mv step, Vsram fix mode and don't update status
	 * when disabling iDVFS
...
/*
 * New SWREQ setting for fixed POS DIV by Fmin
 ...
 * SWREQ new Flow
 * pause -> wait idel -> delay 1us -> set SWREQ -> set fmin ->
 *   delay 1us -> un-pause
 */
...
/*
 * return 0: Ok,
 * return -1: out of range(250~3000),
 * return -2: idvfs cannot be enable
 */
int API_BIGPLLSETFREQ(int Freq)
{
	...
	rg_armpll_sdm_pcw_w((unsigned long long) ((1L << 24) *
				(unsigned long long) freq_sdm_pcm) / 26);
	udelay(1);
...
#if PRINTF_IDVFS
	printf("Bigpll setting ARMPLL_CON0 = 0x%x, ARMPLL_CON1_PTP3 = 0x%x,",
		ptp3_reg_read(ARMPLL_CON0_PTP3),
		ptp3_reg_read(ARMPLL_CON1_PTP3));
	printf("Freq = %dMHz, POS_DIV = %d.\n",
		freq_sdm_pcm, rg_armpll_posdiv_r());
#endif
	return 0;
}
...
int BigSRAMLDOSet(int mVolts_x100)
{
	...
	if (reg_value == 0)
		/* eFuse empty */
		ptp3_reg_write(SRAMLDO_CAL_PTP3, 0x00007777);
	else
		/* write eFuse data */
		ptp3_reg_write(SRAMLDO_CAL_PTP3, reg_value);
	...
	if (mVolts_x100 < 70000)
		...
		reg_value |= (((((mVolts_x100) - 90000) / 2500) + 3));
		/* 90000 ~ 120000 */

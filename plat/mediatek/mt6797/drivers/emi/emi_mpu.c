...
static unsigned char R2, R3, R4, R5, R6, R7, R8, R9, R10;
static unsigned char R11, R12, R13, R14, R15, R16, R17, R18, R19, R20;
static unsigned char R21, R22;
static unsigned int ATF_EMI_MPUI3_2ND, ATF_EMI_MPUJ3_2ND, ATF_EMI_MPUK3_2ND,
			ATF_EMI_MPUL3_2ND;

uint32_t sip_emimpu_set_region_protection(
				...
				unsigned int region_permission)
{
	...
	if ((end != 0) || (start != 0)) {
		...
		if ((*(unsigned int *)(0x10001f00) & 0x2000) == 0)
			emi_physical_offset = 0x40000000;
		else
			emi_physical_offset = 0;
		// printf("atf emi_phy_offset=%llx\n", emi_physical_offset);

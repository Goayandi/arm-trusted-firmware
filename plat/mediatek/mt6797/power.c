...
int power_off_big(const unsigned int linear_id)
{
	...
	while (!(mmio_read_32(SPM_CPU_IDLE_STA2)&(1<<(linear_id+2)))) {
		PRINTF_SPMC("idle sta2 = %x, %x\r",
			mmio_read_32(SPM_CPU_IDLE_STA2), (1<<(linear_id+2)));
	}
...
int power_on_cl3(void)
{
	...
	PRINTF_SPMC("%s before top:%x c0:%x c1:%x\n",
		__func__, big_spmc_status(0x10),
		big_spmc_status(0x01), big_spmc_status(0x02));
...
#define ARMDIV_MON_EN		0x1001A284
	...
	/* sel ckgen_cksw[0] and enable freq meter sel ckgen[13:8],
	 * 01:hd_faxi_ck
	 */
	...
	// mmio_write_32(CLK_MISC_CFG_0, (tmp & 0x00FFFFFF));
...
int big_spmc_sw_pwr_seq_en(int select)
{
	...
	if (select == 0x10) {
		...
		// while (!((mmio_read_32(PTP3_CPUTOP_SPMC) &
		//	FSM_STATE_OUT_MASK)==FSM_ON));
		...
		// while (!((mmio_read_32(PTP3_CPU0_SPMC) &
		//		FSM_STATE_OUT_MASK) == FSM_ON))
		//	;
		// return 0;
		result = 0;
...
void big_sw_off_seq(int select)
{
...

	{
		...
		tmp = mmio_read_32(addr_spmc) & ~(SW_LOGIC_PRE1_PDB |
				SW_LOGIC_PRE2_PDB | SW_LOGIC_PDB |
				SW_SRAM_SLEEPB | (0x3f<<SRAMPD_OFFSET));
		mmio_write_32(addr_spmc, tmp);
				...
				unsigned int value)
{
	...
	if (value == 0) {
		...
		*(unsigned int *)(reg_offset + mcucfg_base) =
								current_value;
	} else if (value == 1) {
		current_value = *(unsigned int *)(reg_offset + mcucfg_base);
		current_value |= (1 << reg_bit);
...
void power_on_little(unsigned int id)
{
	...
	int i;
	unsigned int seq_bit[14] = {
		16, 0, 4, 8, 9, 8, 2, 3, 16, 6, 1, 5, 4, 0
	};
	unsigned int seq_value[14] = {

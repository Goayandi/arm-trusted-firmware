...
char little_on = 0x1;	// [7:0] = core7~core0, core 0 is powered-on by default
int dummy;

void big_spmc_info(void)
{
	PRINTF_SPMC("SwSeq SPMC T:0x%x C0:0x%x C1:0x%x\n",
		big_spmc_status(0x10), big_spmc_status(0x1),
		big_spmc_status(0x2));
}
...
int power_on_big(const unsigned int linear_id)
{
	...
	PRINTF_SPMC("%s before top:%x c0:%x c1:%x\n", __func__,
			big_spmc_status(0x10), big_spmc_status(0x01),
			big_spmc_status(0x02));
	mmio_write_32(CPUCFG, 0xf<<16);	// set BIG poweup on AARCH64

	mmio_write_32(MP2_MISC_CONFIG_BOOT_ADDR_L(linear_id-8),
			(unsigned long)bl31_warm_entrypoint);
	mmio_write_32(MP2_MISC_CONFIG_BOOT_ADDR_H(linear_id-8), 0);

	tmp = mmio_read_32(SPM_MP2_CPU0_PWR_CON+((linear_id-8)<<2)) & ~(1<<0);
	// assert CPUx_PWR_RST_B
	mmio_write_32(SPM_MP2_CPU0_PWR_CON+((linear_id-8)<<2), tmp);
	...
	while (!(mmio_read_32(SPM_CPU_PWR_STATUS) & (1 << (7-(linear_id-8)))))
		; // PWR_ON_ACK_CPU0, wait for 1
	if (!(mmio_read_32(PTP3_CPU0_SPMC + ((linear_id-8)<<2)) & (1<<17)))
		PRINTF_SPMC("SPMC and SPM are dismathed\n");
	while (!(mmio_read_32(PTP3_CPU0_SPMC + ((linear_id-8)<<2)) & (1<<17)))
		; // double check
	tmp = mmio_read_32(SPM_MP2_CPU0_PWR_CON+((linear_id-8)<<2)) | (1<<0);
	// Release CPU0_PWR_RST_B
	mmio_write_32(SPM_MP2_CPU0_PWR_CON+((linear_id-8)<<2), tmp);
...
int power_off_big(const unsigned int linear_id)
{
	...
	while (!(mmio_read_32(SPM_CPU_IDLE_STA2) & (1 << (linear_id + 2))))
		PRINTF_SPMC("idle sta2 = %x, %x\r",
				mmio_read_32(SPM_CPU_IDLE_STA2),
				(1 << (linear_id + 2)));
...
#if FPGA
	// cpu0_sw_pwr_on _override_en
	tmp = mmio_read_32(PTP3_CPU0_SPMC) | (1<<1);
	mmio_write_32(PTP3_CPU0_SPMC, tmp); // De-assert PWR_ON_CPUx
	PRINTF_SPMC("%s waiting ACK\n");
	while (mmio_read_32(PTP3_CPU0_SPMC) & (1<<17))
		; // PWR_ON_ACK_CPU0, wait for 0
	PRINTF_SPMC("%s got ACK\n");
	tmp = mmio_read_32(SPM_MP2_CPU0_PWR_CON+((linear_id-8)<<2)) & ~(1<<0);
	// assert CPUx_PWR_RST_B
	mmio_write_32(SPM_MP2_CPU0_PWR_CON+((linear_id-8)<<2), tmp);
#else
	// With the SPMC, the action should be unnecessary
	// big_spark2_core(linear_id,0);
	tmp = mmio_read_32(SPM_MP2_CPU0_PWR_CON+((linear_id-8)<<2)) & ~(1<<2);
	PRINTF_SPMC("%s set %x to %x\n", __func__,
			SPM_MP2_CPU0_PWR_CON+((linear_id-8)<<2), tmp);
	// De-assert PWR_ON_CPUx
	mmio_write_32(SPM_MP2_CPU0_PWR_CON+((linear_id-8)<<2), tmp);

	PRINTF_SPMC("%s waiting ACK :%x\n", __func__, SPM_CPU_PWR_STATUS);
	while ((mmio_read_32(SPM_CPU_PWR_STATUS) &
		(1 << (7 - (linear_id - 8)))))
		; // PWR_ON_ACK_CPU0, wait for 0
	PRINTF_SPMC("%s got ACK\n", __func__);

	tmp = mmio_read_32(SPM_MP2_CPU0_PWR_CON +
		((linear_id - 8) << 2)) & ~(1<<0);
	PRINTF_SPMC("%s set %x to %x\n", __func__,
		SPM_MP2_CPU0_PWR_CON+((linear_id-8)<<2), tmp);
	// assert CPUx_PWR_RST_B
...
pll_retry:
	...
	while (!(mmio_read_32(SPM_CPU_PWR_STATUS) & (1<<17)))
		; // wait PWR_ON_ACK_CA15MCPUTOP, wait for 1
	if (!(mmio_read_32(PTP3_CPUTOP_SPMC) & (1<<17)))
		PRINTF_SPMC("SPMC and SPM are dismatched\n");
	while (!(mmio_read_32(PTP3_CPUTOP_SPMC) & (1<<17)))
		; // double check
	...
	// ARMPLL_CON0[0], toggle pos div = 0
	mmio_write_32(ARMPLL_CON0, 0x00ff0100);
	udelay(1);

	/* tmp = mmio_read_32(ARMPLL_CON0) | (1<<0);
	 * mmio_write_32(ARMPLL_CON0, tmp);  ARMPLL_CON0[0], Enable PLL
	 */
	// ARMPLL_CON0[0], pos div = 0, Enable PLL
	mmio_write_32(ARMPLL_CON0, 0x00ff0101);
	udelay(10);

	/* SW toggle armpll pos div again */
	// ARMPLL_CON0[0], toggle pos div = 1
	mmio_write_32(ARMPLL_CON0, 0x00ff1101);
	udelay(30);

	/*
	 * Enable abist_clk_en = 1'b1 to monitor this clock
	 *   CLK26CALI_0[12] = 1'b1 (addr: 0x10000220)
	 * 1. Select fqmeter input to ckgen_dbgmux.
	 *    CLK_DBG_CFG[1:0] = 0 (addr: 0x1000010C)
	...
	// pll_div_mux1_sel = 01 = pll clock
	mmio_write_32(PLL_DIV_MUXL_SEL, tmp);
	udelay(1);
...
#define ARMDIV_MON_EN		0x1001A284
	...
	/*
	 * sel ckgen_cksw[0] and enable freq meter sel
	 * ckgen[13:8], * 01:hd_faxi_ck
	 */
	...
	if ((((output > 757500) || (output < 742500)) && (init == 0)) ||
		...
		tmp = mmio_read_32(ARMPLL_CON0) & ~(1<<0);
		// ARMPLL_CON0[0], Disable PLL for workaround
		mmio_write_32(ARMPLL_CON0, tmp);
		udelay(100);

		// Step3: Power off Cluster2
		tmp = mmio_read_32(SPM_MP2_CPUSYS_PWR_CON) & ~(1<<2);
		//De-assert PWR_ON_CA15MCPUTOP
		mmio_write_32(SPM_MP2_CPUSYS_PWR_CON, tmp);
		while ((mmio_read_32(SPM_CPU_PWR_STATUS) & (1<<17)))
			; // wait PWR_ON_ACK_CA15MCPUTOP, wait for 0
	...
	DSB;
	tmp = (1<<2)|(1<<6)|(1<<10);
	while (mmio_read_32(INFRA_TOPAXI_PROTEXTSTA3) & tmp)
		; // wait all of them be 0
...
int power_off_cl3(void)
{
	...
	while (mmio_read_32(MP2_SNOOP_STATUS) & 1)
		; // wait to 0
	...
	// dummy write, avoid optimization
	mmio_write_32((uintptr_t)&power_off_cl3_dummy, tmp);
...
int big_spmc_sw_pwr_seq_en(int select)
{
	...
	if (select == 0x10) {
		...
		// TOP
		tmp = mmio_read_32(addr_spmc) & ~((0x3f<<SRAMPD_OFFSET) |
					SW_LOGIC_PDB | SW_LOGIC_PRE1_PDB |
		...
		// core 0
		tmp = mmio_read_32(addr_spmc) & ~((0x1f<<SRAMPD_OFFSET) |
			SW_LOGIC_PDB | SW_LOGIC_PRE1_PDB | SW_LOGIC_PRE2_PDB |
			SW_SRAM_SLEEPB | SW_SRAM_ISOINTB);
		tmp |= (SW_ISO | SW_HOT_PLUG_RESET);
		mmio_write_32(addr_spmc, tmp);
...
/*
 * "1. if Select invalid, exit/return error -1
 *  2. if Select = CPU0, CPU1, or ALLCPU, read TOP SPMC Status.
 *     If not powered on, Power on TOP first
 * For each Select:
 ...
 */
char sw_pd_cmd[32] = {
	0x01, 0x03, 0x02, 0x06, 0x07, 0x05, 0x04, 0x0c, 0x0d, 0x0f,
...
int big_sw_on_seq(int select)
{
	...
	if (select == 0x1) {
		...
		// Release CPU0_PWR_RST_B
		mmio_write_32(SPM_MP2_CPU0_PWR_CON+((0)<<2), tmp);
	} else if (select == 0x2) {
		tmp = mmio_read_32(SPM_MP2_CPU0_PWR_CON+((1)<<2)) | (1<<0);
		// Release CPU0_PWR_RST_B
		mmio_write_32(SPM_MP2_CPU0_PWR_CON+((1)<<2), tmp);
	} else if (select == 0x10) {
		tmp = mmio_read_32(SPM_MP2_CPUSYS_PWR_CON) & ~(1<<4);
		// Release clk_dis
		mmio_write_32(SPM_MP2_CPUSYS_PWR_CON, tmp);
	...
	for (i = 0; i < nb_srampd; i++) {
		tmp = orig_sram_pd | (sw_pd_cmd[i] << S RAMPD_OFFSET);
		mmio_write_32(addr_spmc, tmp);
	}
	...
	if (select == 0x10) {
		...
		while ((mmio_read_32(INFRA_TOPAXI_PROTEXTSTA3) & (1<<10)))
			; // bit[10]:pwrdnackn waiting ack,wait for 0
		big_spmc_info();
...
{
	// power on a core before power on the TOP,
	// we power on TOP automatically
	...
	case 0x2:
		...
		big_spmc_info();
#endif
		//set BIG poweup on AARCH64
		mmio_write_32(0x10222208, 0xf << 16);
		unsigned int add_idx = select - 1;
		mmio_write_32(MP2_MISC_CONFIG_BOOT_ADDR_L(add_idx),
				(unsigned long) bl31_on_entrypoint);
		mmio_write_32(MP2_MISC_CONFIG_BOOT_ADDR_H(add_idx), 0);
		PRINTF_SPMC("mt_on_2, entry H:%x L:%x\n",
			mmio_read_32(MP2_MISC_CONFIG_BOOT_ADDR_H(add_idx)),
			mmio_read_32(MP2_MISC_CONFIG_BOOT_ADDR_L(add_idx)));

		// PRINTF_SPMC("Write 0x10208008=%d\n",8+add_idx);
		// discussed with Scott
		// mmio_write_32(0x10208008, 8+add_idx);
#if SPMC_DVT
		udelay(SPMC_DVT_UDELAY);
		big_spmc_info();
#endif
...
void big_sw_off_seq(int select)
{
...

	{
		if (select == 0x10) {
			...
			PRINTF_SPMC("CL3 is in WFI\n");
			tmp = mmio_read_32(PLL_DIV_MUXL_SEL) & ~(1<<0);
			//pll_div_mux1_sel = 26MHZ
			mmio_write_32(PLL_DIV_MUXL_SEL, tmp);
			tmp = mmio_read_32(ARMPLL_CON0) & ~(1<<0);
			//ARMPLL_CON0[0], Disable PLL
			mmio_write_32(ARMPLL_CON0, tmp);
		} else {
			PRINTF_SPMC("Wait CPU WFI mask:%x\n", select);
			while (!(mmio_read_32(SPM_CPU_IDLE_STA2) &
					(1<<(9+select))))
				;
		...
		tmp = mmio_read_32(addr_spmc) &
			~(SW_LOGIC_PRE1_PDB | SW_LOGIC_PRE2_PDB |
				SW_LOGIC_PDB | SW_SRAM_SLEEPB |
				(0x3f<<SRAMPD_OFFSET));
		mmio_write_32(addr_spmc, tmp);

		while ((mmio_read_32(addr_spmc) &
				SW_LOGIC_PRE1_PDBO_ALL_ON_ACK))
			; // wait for 0
		while ((mmio_read_32(addr_spmc) &
			SW_LOGIC_PRE2_PDBO_ALL_ON_ACK))
			; // wait for 0
		while (!(mmio_read_32(addr_spmc) & SW_LOGIC_PDBO_ALL_OFF_ACK))
			; //wait for 1
...
int big_spmc_sw_pwr_cntrl_disable(int select)
{
	...
	case 0x1:
		if (mmio_read_32(PTP3_CPU0_SPMC) & SW_FSM_OVERRIDE) {
			...
			tmp = mmio_read_32(PTP3_CPU0_SPMC) &
	...
	case 0x2:
		if (mmio_read_32(PTP3_CPU1_SPMC) & SW_FSM_OVERRIDE) {
			...
			tmp = mmio_read_32(PTP3_CPU1_SPMC) &
				~SW_PWR_ON_OVERRIDE_EN;
...
static void mcucfg_set_bit(unsigned long long reg_offset,
				...
				unsigned int value)
{
	...
	if (value == 0) {
		...
		*(unsigned int *)(reg_offset + mcucfg_base) = current_value;
	} else if (value == 1) {
		current_value = *(unsigned int *)(reg_offset + mcucfg_base);
		current_value |= (1 << reg_bit);
		*(unsigned int *)(reg_offset + mcucfg_base) = current_value;
...
static unsigned int pwr_get_bit(unsigned long long reg_offset,
				unsigned int reg_bit)
{
	volatile unsigned int current_value;
	// PRINTF_SPMC("%s reg:%x\n",__func__,reg_offset + SPM_BASE);
	current_value = *(unsigned int *)(reg_offset + SPM_BASE);

	return (((current_value) & (1 << reg_bit)) >> reg_bit);
}

static void pwr_set_bit(unsigned long long reg_offset,
			unsigned int reg_bit, unsigned int value)
{
	unsigned int current_value;
	// PRINTF_SPMC("%s reg:%x\n",__func__,reg_offset + SPM_BASE);
	...
	if (value == 0) {
		current_value = *(unsigned int *)(reg_offset + SPM_BASE);
		current_value &= (~(1 << reg_bit));
		*(unsigned int *)(reg_offset + SPM_BASE) = current_value;
	} else if (value == 1) {
		current_value = *(unsigned int *)(reg_offset + SPM_BASE);
		current_value |= (1 << reg_bit);
		*(unsigned int *)(reg_offset + SPM_BASE) = current_value;
...
void power_on_little_cl(unsigned int cl_idx)
{
	...
	if (!cl_idx) {
		tmp = mmio_read_32(INFRA_TOPAXI_PROTECTEN1) &
					~((1<<0)|(1<<4)|(1<<8)|(1<<11));
		mmio_write_32(INFRA_TOPAXI_PROTECTEN1, tmp);
		DSB;
...
void power_off_little(unsigned int id)
{
	...
	if (id < 4)
		...
		while (!(mmio_read_32(MP1_CA7L_MISC_CONFIG) &
			(1<<(24+(id-4)))))
			;

	// offset in SPM group, checked!
	unsigned int reg_offset = 0x220 + (id<<2);
	int i;
	...
	// confirm the core is not in the retention mode,
	// bit[x]:core x, wait 0
...
void power_off_little_cl(unsigned int cl_idx)
{
	...
	if (!cl_idx) {
		...
		// MP0_AXI_CONFIG
		mcucfg_set_bit(0x002C, 4, 1);
	} else {
		cci_disable_snoop_dvm_reqs(1);
		// MP1_AXI_CONFIG
		mcucfg_set_bit(0x022C, 4, 1);
	}

	while ((mmio_read_32(0x10006174)&(1<<(20+cl_idx))) != (1<<(20+cl_idx)))
		; // wait MP1_STANDBYWFIL2 to high
	...
	if (!cl_idx) {
		...
		DSB;
		tmp = (1<<0) | (1<<4) | (1<<8) | (1<<11);
		while ((mmio_read_32(INFRA_TOPAXI_PROTEXTSTA3) & tmp) != tmp)
			; // wait all of them be 1
	} else {
		tmp = mmio_read_32(INFRA_TOPAXI_PROTECTEN1) |
					((1<<1)|(1<<5)|(1<<9));
		mmio_write_32(INFRA_TOPAXI_PROTECTEN1, tmp);
...
int little_spark2_setldo(unsigned int core)
{
	...
	tmp = (mmio_read_32(base_vret) & ~((0x3f) << (offset<<3))) |
		(sparkvretcntrl << (offset<<3));

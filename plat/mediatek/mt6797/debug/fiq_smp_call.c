...
struct call_function_data cfd[PLATFORM_CORE_COUNT];

int fiq_smp_call_function(unsigned int map, inter_cpu_call_func_t func,
				void *info, int wait)
{
	int cpu;
	int lockval, tmp;
	...
	for (cpu = 0; cpu < PLATFORM_CORE_COUNT; cpu++) {
		if (map & (1 << cpu)) {
			printf("Wait till cpu %d is ready for inter-cpu call\n",
				cpu);
			...
			__asm__ volatile(
				...
				" add %w0, %w0, %w3\n"
				" stxr  %w1, %w0, [%2]\n"
				" cbnz  %w1, 1b" : "=&r" (lockval),
				"=&r" (tmp) : "r" (&(cfd[cpu].lock)),
				"Ir" (1) : "cc");
